...
const useStyles = makeStyles({
  content: { display: 'flex', flexDirection: 'column', gap: '4px', maxHeight: '400px', overflow: 'auto' },
  row: { display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 8px', borderRadius: '4px' },
            ...
            <div className={styles.content}>
              {results.map((r, i) => (
                <div key={i} className={`${styles.row} ${r.success ? styles.successRow : styles.errorRow}`}>
                  ...
                  <Text size={200} weight="semibold">{r.flowName}</Text>
                  <Text size={200}>→ {r.targetName}</Text>
                  {!r.success && r.errorMessage && (
                    <Text size={200} style={{ color: tokens.colorPaletteRedForeground1 }}>{r.errorMessage}</Text>
                  )}
                </div>
              ))}
            </div>
